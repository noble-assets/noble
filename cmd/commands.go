...
func newApp(logger log.Logger, db dbm.DB, traceStore io.Writer, appOpts servertypes.AppOptions) servertypes.Application {
	...
	app, err := app.NewNobleApp(logger, db, traceStore, true, appOpts, baseappOptions...)
	if err != nil {
		panic(err)
	}
...
func appExport(
	...
	if height != -1 {
		miniApp, err = app.NewNobleApp(logger, db, traceStore, false, appOpts)
		if err != nil {
		...
		miniApp, err = app.NewNobleApp(logger, db, traceStore, true, appOpts)
		if err != nil {
			return servertypes.ExportedApp{}, err
		}
	}
