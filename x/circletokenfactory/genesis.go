...
func InitGenesis(ctx sdk.Context, k *keeper.Keeper, bankKeeper types.BankKeeper, genState types.GenesisState) {
	...
	if genState.MintingDenom != nil {
		...
		if !found {
			panic(sdkerrors.Wrapf(types.ErrDenomNotRegistered, "circletokenfactory minting denom %s is not registered in bank module denom_metadata", genState.MintingDenom.Denom))
		}
