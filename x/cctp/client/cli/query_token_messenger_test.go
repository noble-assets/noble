...
func networkWithTokenMessengerObjects(t *testing.T, n int) (*network.Network, []types.TokenMessenger) {
	...
	for i := 0; i < n; i++ {
		attester := types.TokenMessenger{
			DomainId: uint32(i),
		}
		nullify.Fill(&attester)
		state.TokenMessengerList = append(state.TokenMessengerList, attester)
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.NotNil(t, resp.TokenMessenger)
				require.Equal(t,
					nullify.Fill(&tc.obj),
