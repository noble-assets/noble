...
func TestGenCreation(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	...
	fn := noble.FullNodes[0]

	fmt.Println("number of Vals: ", len(noble.Validators))

	for x := range noble.Validators {
		valAddress, _ := noble.Validators[x].AccountKeyBech32(ctx, "validator")
		if err != nil {
			fmt.Println("ERorror! ", err)
		}
		fn.ExecTx(ctx, authorityKeyName, "staking", "delegate", valAddress, "1stake")
	}
	...
	state, err := fn.ExportState(ctx, 0)
	if err != nil {
