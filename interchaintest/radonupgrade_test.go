...
func TestRadonUpgrade(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	...
	a, _, err := noble.Validators[0].ExecQuery(ctx, "globalfee", "parameters")
	require.NoError(t, err, "error querying globalfee params")

	var globalFeeParams globalfeetypes.Params
	err = json.Unmarshal(a, &globalFeeParams)
