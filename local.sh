...
if ! [ -f .duke/data/priv_validator_state.json ]; then
  ...
  touch $TEMP && jq '.app_state.dollar.portal.peers = { "10002": { "transceiver": "AAAAAAAAAAAAAAAAKcvx4HFm0xRGMHrgeZn6bRYiOZA=", "manager": "AAAAAAAAAAAAAAAAG3rhlLIMVVudmZyDX3TNzjamenQ=" } }' .duke/config/genesis.json > $TEMP && mv $TEMP .duke/config/genesis.json
  ...
  touch $TEMP && jq '.app_state.wormhole.guardian_sets = {"0":{"addresses":["E5R71IsY5T/a7ud/NHM5Gscnxjg="],"expiration_time":0}}' .duke/config/genesis.json > $TEMP && mv $TEMP .duke/config/genesis.json
